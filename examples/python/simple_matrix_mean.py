...
def run_row_sum_example(crypto_context, keys, ctm_x, matrix):
    ...
    if ctm_x.order == onp.ROW_MAJOR:
        ctm_x.extra["rowkey"] = onp.sum_row_keys(keys.secretKey, ctm_x.ncols, ctm_x.batch_size)
...
def run_column_sum_example(crypto_context, keys, ctm_x, matrix):
    ...
    if ctm_x.order == onp.ROW_MAJOR:
        ...
        ctm_x.extra["rowkey"] = onp.sum_row_keys(keys.secretKey, ctm_x.nrows, ctm_x.batch_size)
    else:
        raise ValueError("Invalid order.")
