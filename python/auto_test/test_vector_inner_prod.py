...
def fhe_vector_add(ringDimension, mult_depth, a, b, precision=2):
    ...
    a = np.array(a)
    b = np.array(b)
    cta = fp.array(cc, a, total_slots, pub_key=keys.publicKey)
    ctb = fp.array(cc, b, total_slots, pub_key=keys.publicKey)
    result = fp.dot(cc, cta, ctb).decrypt(cc, keys.secretKey)
...
class TestVectorInnerProduct(unittest.TestCase):
    @classmethod
    ...
    for r in ring_dims:
        for size in sizes:
            for _ in range(2):
                ...
                method = TestVectorInnerProduct.generate_test(name, r, a, b, expected)
                setattr(TestVectorInnerProduct, name, method)
                test_counter += 1
