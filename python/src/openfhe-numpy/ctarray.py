...
class CTarray:
    ...
    def __init__(
        ...
        ndim: int,
        size: int,  # todo size -> (batch_size) = ncols * nrows (available slots that you can use )
        ncols: int = 1,
        ...
        size : int
            The total number of slots (after padding).
        ncols : int, optional
    ...
    @property
    def info(self):
        return [None, self.original_shape, self.ndim, self.size, self.ncols, self.encoding_order]

    def copy(self):
        return CTarray(
            self.data, self.original_shape, self.ndim, self.size, self.ncols, self.encoding_order
        )
