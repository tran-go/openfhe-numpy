...
def fhe_vector_add(ringDimension, mult_depth, a, b, precision=2):
    ...
    result = fp.transpose(cc, cta, ctb).decrypt(cc, keys.secretKey)
    ...
    for r in ring_dims:
        for size in sizes:
            for _ in range(2):
                ...
                expected = np.transpose(matrix).tolist()
                name = f"test_case_{test_counter}"
                method = TestMatrixTranspose.generate_test(name, r, a, b, expected)
                setattr(TestMatrixTranspose, name, method)
                test_counter += 1
